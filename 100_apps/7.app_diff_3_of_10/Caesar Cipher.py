...
def caesar(simple_text, shift_amount, direction):
    cipher_text = ''
    flag = -1
            ...
            for i, char in enumerate(alphabet):
                if letter == char:
                    ...
                    # If index + shift amount more than length alphabet, then we need to nullify our indexes
                        ...
                        cipher_text += alphabet[i + amount*flag*26 + shift_amount]
    print(f'The {direction} word is: {cipher_text}')
...
while flag == False:
    ...
    restart = input('Type \'yes\' if you want to start again. Othewise type \'no\'\n')
    if restart == 'yes':
        pass
    elif restart == 'no':
        flag = True
